...
)
{
    ...
    mesh->vertices =
        (vertex_t*) malloc(vertex_count * sizeof(vertex_t));
    memcpy(
        mesh->vertices,                  // target
        vertices,                        // source
        vertex_count * sizeof(vertex_t)  // size in bytes
    );
    ...
    mesh->renderable = renderable_create(
        vertices, vertex_count, indices, index_count, albedo_texture
    );
...
glm::vec3 mesh_calculate_bounding_box(mesh_t* self)
{
    ...
    for (int i = 0; i < self->vertex_count; i++) {
        ...
        min_bounds =
            glm::min(min_bounds, glm::vec3(pos.x, pos.y, pos.z));
        max_bounds =
            glm::max(max_bounds, glm::vec3(pos.x, pos.y, pos.z));
    }
...
glm::vec3 mesh_calculate_center_shift(mesh_t* self, glm::vec3 scale)
{
    ...
    // Calculate the center shift as the difference between the center
    // of mass and the center of the bounding box (assuming that the
...
mesh_t* mesh_shape_rectangular_prism_create(glm::vec3 dimensions)
{
    ...
    GLuint indices[] = {
        0,  1,  2,  2,  3,  0,   // Front face (two triangles)
        4,  5,  6,  6,  7,  4,   // Back face (two triangles)
        8,  9,  10, 10, 11, 8,   // Top face (two triangles)
        12, 13, 14, 14, 15, 12,  // Bottom face (two triangles)
        16, 17, 18, 18, 19, 16,  // Left face (two triangles)
        20, 21, 22, 22, 23, 20   // Right face (two triangles)
    };

    return mesh_create(
        "name of mesh",                       // name
        (vertex_t*) &vertices, (GLsizei) 24,  // vertices
        (GLuint*) &indices, (GLsizei) 36,     // indices
        (texture_t*) NULL                     // texture
    );
}
