...
physics_t* physics_create()
{
    ...
    self->dispatcher =
        new btCollisionDispatcher(self->collision_configuration);
    ...
    self->dynamics_world = new btDiscreteDynamicsWorld(
        self->dispatcher, self->overlapping_pair_cache, self->solver,
        self->collision_configuration
    );

	self->dynamics_world->setGravity(
		// btVector3(0, -1.625f, 0) // Moon gravity
		btVector3(0, -9.8, 0) // earth gravity
	);
  return self;
}
...
void physics_delete(physics_t* self)
{
    ...
    const int num_collision_objects =
        self->dynamics_world->getNumCollisionObjects();
    for (int i = num_collision_objects - 1; i >= 0; i--) {
        btCollisionObject* obj =
            self->dynamics_world->getCollisionObjectArray()[i];
...
/* *********************************************************************
 * Step simulation, elapse time in seconds
 * ********************************************************************/
void physics_step_simulation(physics_t* self, float time_step)
{
    const float iterations_per_step = 10;
    self->dynamics_world->stepSimulation(
        time_step, iterations_per_step
    );
}