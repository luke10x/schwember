...
void ctx_load(ctx_t* ctx, int width, int height) {
  ...
  // Setup shaders
  ctx->default_shader =
      shader_create("src/shaders/default.vert", "src/shaders/default.frag");
  ctx->light_shader =
      shader_create("src/shaders/light.vert", "src/shaders/light.frag");
  ctx->sky_shader =
      shader_create("src/shaders/default.vert", "src/shaders/sky.frag");
  ...
  ctx->floor_transform =
      glm::translate(glm::mat4(1.0f), glm::vec3(0, 0.01f, 0)); // one cm up
  // collider_t* floor_collider = collider_create_plane(ctx->floor_transform,
  // ctx->physics);
  ...
  for (int i = 0; i < ctx->map_model->num_meshes; i++) {
    ...
    collider_t* map_collider =
        collider_create_mesh(ctx->map_transform, mesh, ctx->physics);
    ((collider_sphere_t*)map_collider)->rigid_body->setRestitution(0.2);
  ...
  ctx->pc = pc_create(ctx->default_shader, ctx->light_shader, ctx->physics);
  ...
  ctx->sphere_transform =
      glm::translate(glm::mat4(1.0f), glm::vec3(0, 8, 20 - 20));
  ctx->sphere_transform =
      glm::scale(ctx->sphere_transform, glm::vec3(0.5f, 0.5f, 0.5f));
  ctx->sphere_collider =
      collider_create_sphere(ctx->sphere_transform, 1.0f, ctx->physics);
  btVector3 force(-2.5f, 0.2f, -0.4f);
  ((collider_sphere_t*)ctx->sphere_collider)
      ->rigid_body->applyCentralImpulse(force);
  ((collider_sphere_t*)ctx->sphere_collider)->rigid_body->setRestitution(0.8);
...
inline static void ctx_render(ctx_t* ctx) {
  ...
  // Pyramid render
  ctx->pyramid_transform =
      collider_update_transform(ctx->pyramid_collider, ctx->pyramid_transform);
  shader_set_uniform_mat4(ctx->default_shader, "modelToWorld",
                          ctx->pyramid_transform);
  renderable_draw(ctx->pyramid->renderable, ctx->default_shader, ctx->camera);

  // Myramid render
  ctx->myramid_transform =
      collider_update_transform(ctx->myramid_collider, ctx->myramid_transform);
  shader_set_uniform_mat4(ctx->default_shader, "modelToWorld",
                          ctx->myramid_transform);
  ...
  ctx->sphere_transform =
      collider_update_transform(ctx->sphere_collider, ctx->sphere_transform);
  ...
  // );
  ctx->pc->transform =
      collider_update_transform(ctx->pc->collider, ctx->pc->transform);
