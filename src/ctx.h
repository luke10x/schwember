...
void ctx_load(ctx_t* ctx, int width, int height)
{
    ...
    shader_set_uniform_vec4(
        ctx->light_shader, "lightColor", light_color
    );
    shader_set_uniform_vec4(
        ctx->default_shader, "lightColor", light_color
    );
    ...
    shader_set_uniform_mat4(
        ctx->light_shader, "modelToWorld", lamp_transform
    );
    ...
    ctx->pyramid_transform =
        glm::translate(glm::mat4(1.0f), glm::vec3(-2, 4, 4));
    ctx->pyramid_transform =
        glm::scale(ctx->pyramid_transform, glm::vec3(1.5f, 1.5f, 1.5f));
    ctx->pyramid_collider = collider_create_box_from_mesh(
        ctx->pyramid_transform, ctx->pyramid, ctx->physics
    );

    // Myramid
    ctx->myramid = mesh_sample_create_pyramid();
    ctx->myramid_transform =
        glm::translate(glm::mat4(1.0f), glm::vec3(-3.1, 7, 4));
    ctx->myramid_transform =
        glm::scale(ctx->myramid_transform, glm::vec3(3.0f, 3.0f, 3.0f));
    ctx->myramid_collider = collider_create_box_from_mesh(
        ctx->myramid_transform, ctx->myramid, ctx->physics
    );

    // Floor is in the middle
    ctx->floor           = mesh_sample_create_floor();
    ctx->floor_transform = glm::translate(
        glm::mat4(1.0f), glm::vec3(0, 0.01f, 0)
    );  // one cm up
    // collider_t* floor_collider =
    // collider_create_plane(ctx->floor_transform, ctx->physics);

    glm::mat4 skybox_transform =
        glm::translate(glm::mat4(1.0f), glm::vec3(-0.5, -0.5, 0.0f));
    shader_set_uniform_mat4(
    ...
    for (int i = 0; i < ctx->map_model->num_meshes; i++) {
        ...
        ((collider_sphere_t*) map_collider)
            ->rigid_body->setRestitution(0.2);
    ...
    ctx->pc =
        pc_create(ctx->default_shader, ctx->light_shader, ctx->physics);
    ...
    ctx->sphere_transform =
        glm::scale(ctx->sphere_transform, glm::vec3(0.5f, 0.5f, 0.5f));
    ctx->sphere_collider = collider_create_sphere(
        ctx->sphere_transform, 1.0f, ctx->physics
    );
    btVector3 force(-2.5f, 0.2f, -0.4f);
    ((collider_sphere_t*) ctx->sphere_collider)
        ->rigid_body->applyCentralImpulse(force);
    ((collider_sphere_t*) ctx->sphere_collider)
        ->rigid_body->setRestitution(0.8);
...
inline static void ctx_render(ctx_t* ctx)
{
    ...
    ctx->physics->dynamics_world->contactTest(
        ((collider_sphere_t*) ctx->sphere_collider)->rigid_body, my_ccb
    );

    // Pyramid render
    ctx->pyramid_transform = collider_update_transform(
        ctx->pyramid_collider, ctx->pyramid_transform
    );
    shader_set_uniform_mat4(
        ctx->default_shader, "modelToWorld", ctx->pyramid_transform
    );
    renderable_draw(
        ctx->pyramid->renderable, ctx->default_shader, ctx->camera
    );

    // Myramid render
    ctx->myramid_transform = collider_update_transform(
        ctx->myramid_collider, ctx->myramid_transform
    );
    shader_set_uniform_mat4(
        ctx->default_shader, "modelToWorld", ctx->myramid_transform
    );
    renderable_draw(
        ctx->myramid->renderable, ctx->default_shader, ctx->camera
    );

    // Floor render
    shader_set_uniform_mat4(
        ctx->default_shader, "modelToWorld", ctx->floor_transform
    );
    renderable_draw(
        ctx->floor->renderable, ctx->default_shader, ctx->camera
    );

    shader_set_uniform_mat4(
        ctx->default_shader, "modelToWorld", glm::mat4(1.0)
    );
    ...
    shader_set_uniform_mat4(
        ctx->default_shader, "modelToWorld", ctx->sign_transform
    );
    model_draw(ctx->sign, ctx->default_shader, ctx->camera);

    shader_set_uniform_mat4(
        ctx->default_shader, "modelToWorld", ctx->subject_transform
    );
    model_draw(ctx->subject, ctx->default_shader, ctx->camera);

    shader_set_uniform_mat4(
        ctx->default_shader, "modelToWorld", ctx->suzanne_transform
    );
    ...
    shader_set_uniform_mat4(
        ctx->default_shader, "modelToWorld", ctx->sphere_transform
    );
    model_draw(ctx->sphere, ctx->default_shader, ctx->camera);

    // transparent must render last
    renderable_draw(
        ctx->lamp->renderable, ctx->light_shader, ctx->camera
    );

    // main character render

    // ctx->pc->bumper_transform = collider_update_transform(
    //     ctx->pc->bumper_collider,
    //     ctx->pc->bumper_transform
    // );
    ctx->pc->transform = collider_update_transform(
        ctx->pc->collider, ctx->pc->transform
    );
